...
public class SuperSaiyanScrollView extends FrameLayout 
    ...
    private boolean mAutoDrawableWidth;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mAutoDrawableWidth = typedArray.getBoolean(R.styleable.SuperSaiyanScrollView_ssjn_overlayAutoWidth, false);
        	...
        	mAutoDrawableWidth = false;	//disabled by default
    ...
    public void draw(Canvas canvas) {
        ...
        //if mOverlayWidth was not updated before
        if (mOverlayWidth == -1) {
        	measureOverlayWidth();
        }
	...
	public void refresh() {
		getSections();
		if (mAutoDrawableWidth) { measureOverlayWidth(); }
		mDrawOverlay = false; // avoids temporary content flash of previous sections
	}
	
	private void measureOverlayWidth() {
		if (mPaint == null || mList == null) return;	//will be updated next time..
		if (mSections == null) {
			getSections();
		}
		float maxTextWidth = Float.MIN_VALUE;
		for (Object section : mSections) {
			float w = mPaint.measureText(section.toString());
			if (maxTextWidth < w)
				maxTextWidth = w;
		}
		int padding = (int) (mPaint.descent() * 2 + 32); // 32 compensates
															// drawable padding
		mOverlayWidth = (int) (maxTextWidth) + padding;
	}
    ...
    public void setOverlaySizeScheme(OverlaySizeScheme scheme, boolean autoWidth) {
    	mAutoDrawableWidth = autoWidth;
